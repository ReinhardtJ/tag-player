...
fn test_gather_music_library_non_existent_folder() {
    let library = gather_music_library(Path::new("/nonexistent"));
...
fn test_gather_music_library_empty_folder() {
    let library = gather_music_library(Path::new("./tests/music_libraries/empty"));
    assert_eq!(library.songs.len(), 0);
    assert_eq!(library.errors.len(), 0);
}
#[test]
fn test_gather_music_library_one_file_with_tags() {
    let library = gather_music_library(Path::new("./tests/music_libraries/one_file_with_tags"));
